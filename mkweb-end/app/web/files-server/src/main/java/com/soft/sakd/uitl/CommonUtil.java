...
/**
 * Description: 通用工具类 Author: panyt Version: 1.0 Create Date Time: 2017-09-04 9:39. Update Date
 * Time: 2017-09-04 9:39.
 *
 * @see CommonUtil
 */
public class CommonUtil {
  ...
  public static <K, V> Map<String, V> toMap(Object... data) {
    ...
    if (data.length % 2 == 1) {
      IllegalArgumentException e =
          new IllegalArgumentException("你必须传入一个键值对应的序列 (长度 = " + data.length + ")");
      throw e;
    }
  ...
  public static <K, V> Map<K, V> createMap(List<K> keys, List<V> values) {
    if (keys == null || values == null || keys.size() != values.size()) {
      throw new IllegalArgumentException(
          "请确认keys和values不为空，并且两个list长度相同（keys:" + keys.size() + ", values:" + values.size() + "）");
    }
  ...
  public static String getProperty(String key, String fileName, String defaultValue) {
    ...
    try {
      InputStream in =
          Thread.currentThread().getContextClassLoader().getResourceAsStream("config/" + fileName);
      prop.load(in);
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  public static String searchUniqueNumber(String num, String prefixion) {
    ...
    if (isNotEmpty(num)) {
      if (num.contains(prefixion)) {
        number =
            String.valueOf(Integer.parseInt(num.substring(prefixion.length(), num.length())) + 1);
      } else {
        number = "001";
      }
      try {
